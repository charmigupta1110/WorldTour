<?php
 include 'cn.php';
 	...
 	if(isset($_POST['btn'])){
 	$d=$_POST['des'];
 	$depart_in=$_POST['d_in'];
 	$travellers=$_POST['t'];
 	$class=$_POST['class'];
 	$flight_name=$_POST['flight_name'];
 	$flight_time=$_POST['flight_time'];
 		if(empty($d)){
 			$err['d']="Please Select Your Destination";
 		}
 		if(empty($depart_in)){
 			$err['depart_in']="Please Select Your Date";
 		}
 		
 		if(empty($travellers)){
 			$err['travellers']="Please Enter Travellers";
 		}
 		if(empty($class)){
 			$err['class']="Please Select Your class";
 		}
 		if(empty($flight_name)){
 			$err['flight_name']="Select flight";
 		}
 		if(empty($flight_time)){
 			$err['flight_time']="Select time of flight";
 		}
 		if(!isset($err)){
 			$insert=mysqli_query($cn,"insert into Flight(destination,f_from,class,travellers,flight_name,flight_time) value('$d','$depart_in','$class','$travellers','$flight_name','$flight_time')");
 		}
...
<div id="example3">
<h1 align= "center" style="color:white;" > SEARCH FLIGHTS </h1>
<h1 align= "center" style="color:white;" > Search And Book Flights With No Booking Fees </h1>

<form method="post">
<h4 align="left">
<label for ="name" > Origin:Mumbai</label>
<br>
...
<span><?php if(isset($err)){echo $err['d'];} ?></span>
<br>
<label for= "in"> Depart-date: </label>
<input type="date" name="d_in">
<span><?php if(isset($err)){echo $err['depart_in'];} ?></span>
<br>
<label for="flight_name">Flight Name</label>
<select name="flight_name">
...
<span><?php if(isset($err)){echo $err['flight_name'];} ?></span>
<br>
...
<span><?php if(isset($err)){echo $err['flight_time'];} ?></span>
<br>
...
<a href="display.php"> Check Flight Booking </a>
<div class="media">
<h1 style="color:black; margin-bottom:100px;"> AmericanAirlines
Qantas
